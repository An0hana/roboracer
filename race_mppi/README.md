...
```bash
ros2 run race_mppi generate_raceline.py centerline.csv raceline.csv \
...
```bash
ros2 run race_mppi extract_raceline_from_map.py \
...
```bash
colcon build --packages-select race_mppi --cmake-args -DCMAKE_BUILD_TYPE=Release
source install/setup.bash
ros2 launch race_mppi race_mppi.launch.py \
...
source install/setup.bash
colcon build --packages-select race_mppi \
  --cmake-args -DCMAKE_BUILD_TYPE=Release \
               -DRACE_MPPI_ENABLE_CUDA=ON \
               -DRACE_MPPI_CUDA_ARCHITECTURE=87
```
...
- Upstream v0.9.0 error macros terminate the process for some internal CUDA failures. Run the
  controller behind the independent `race_safety` watchdog and validate process-restart
