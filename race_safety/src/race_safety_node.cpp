...
class RaceSafetyNode : public rclcpp::Node
{
public:
  RaceSafetyNode()
  : Node("race_safety")
  {
    ...
    scan_subscription_ = create_subscription<sensor_msgs::msg::LaserScan>(
      ...
      std::bind(&RaceSafetyNode::scanCallback, this, std::placeholders::_1));
    odom_subscription_ = create_subscription<nav_msgs::msg::Odometry>(
      odom_topic, rclcpp::QoS(10),
      std::bind(&RaceSafetyNode::odomCallback, this, std::placeholders::_1));
    ...
    parameter_callback_handle_ = add_on_set_parameters_callback(
      std::bind(&RaceSafetyNode::parametersCallback, this, std::placeholders::_1));

    const auto period = std::chrono::duration<double>(1.0 / publish_rate);
    timer_ = create_wall_timer(
      std::chrono::duration_cast<std::chrono::nanoseconds>(period),
      std::bind(&RaceSafetyNode::timerCallback, this));
...
private:
  ...
  void odomCallback(const nav_msgs::msg::Odometry::ConstSharedPtr message)
  {
    ...
    std::lock_guard<std::mutex> lock(core_mutex_);
    double steering = core_->currentSteeringAngle();
    ...
    core_->updateState(speed, steering, sourceTimeOrNow(message->header.stamp));
  }
    ...
    const std::vector<rclcpp::Parameter> & parameters)
  {
    ...
    for (const auto & parameter : parameters) {
      if (parameter.get_name() != "controller_mode") {
        ...
        result.reason = "race_safety parameters are immutable after startup";
        return result;
      }
      try {
    ...
    const ArbitrationResult & result, const builtin_interfaces::msg::Time & stamp)
  {
    ...
    status.name = "race_safety/arbiter";
  ...
  double steering_estimation_min_speed_{0.050};
...
int main(int argc, char ** argv)
{
  ...
  try {
    rclcpp::spin(std::make_shared<race_safety::RaceSafetyNode>());
  } catch (const std::exception & error) {
    RCLCPP_FATAL(rclcpp::get_logger("race_safety"), "Startup failed: %s", error.what());
    rclcpp::shutdown();
    return 1;
  }
  rclcpp::shutdown();
