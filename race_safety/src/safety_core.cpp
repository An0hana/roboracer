...
AebAssessment SafetyCore::assessAeb() const
{
  ...
  for (std::size_t beam = 0U; beam < scan_.ranges.size(); ++beam) {
    ...
    const double range = std::max(measured_range, scan_.range_min);
...
VescCommandMapper::VescCommandMapper(const VescMappingConfig & config)
: config_(config)
{
  if (!std::isfinite(config_.speed_to_erpm_gain) ||
    !std::isfinite(config_.speed_to_erpm_offset) ||
    !std::isfinite(config_.steering_center) ||
    !std::isfinite(config_.steering_left_gain) ||
    !std::isfinite(config_.steering_right_gain) ||
    !std::isfinite(config_.servo_min) || !std::isfinite(config_.servo_max) ||
    config_.servo_min >= config_.servo_max ||
    config_.steering_center < config_.servo_min ||
    config_.steering_center > config_.servo_max ||
    !std::isfinite(config_.erpm_min) || !std::isfinite(config_.erpm_max) ||
    config_.erpm_min >= config_.erpm_max)
  {
    throw std::invalid_argument("invalid VescMappingConfig");
  }
}

VescOutput VescCommandMapper::map(const DriveCommand & command) const
{
  if (!command.valid()) {
    return VescOutput{0.0, config_.steering_center, false};
  }

  const double steering_gain = command.steering_angle >= 0.0 ?
    config_.steering_left_gain : config_.steering_right_gain;
  const double servo = std::clamp(
    config_.steering_center + steering_gain * command.steering_angle,
    config_.servo_min, config_.servo_max);
  const double erpm = std::clamp(
    config_.speed_to_erpm_gain * command.speed + config_.speed_to_erpm_offset,
    config_.erpm_min, config_.erpm_max);
  return VescOutput{erpm, servo, true};
}

VescOutput VescCommandMapper::stop(double steering_angle) const
{
  const VescOutput mapped = map(DriveCommand{0.0, steering_angle});
  return VescOutput{0.0, mapped.servo_position, mapped.valid};
}
